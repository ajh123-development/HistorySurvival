...
public class CuckooServiceTest {
    public static void main(String[] args) {
        ...
        CLKernel testCuckooKernel = clCreateKernel(kernelHolder.getKernel(KernelNames.TEST_CUCKOO), "testCuckoo", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());

        clSetKernelArg(testCuckooKernel, 0, inputArgument);
        clSetKernelArg(testCuckooKernel, 1, resultOutBuffer.getMem());
        clSetKernelArg(testCuckooKernel, 2, cuckooHashService.getTable().getMem());
        clSetKernelArg(testCuckooKernel, 3, cuckooHashService.getStash().getMem());
        clSetKernelArg(testCuckooKernel, 4, cuckooHashService.getPrime());
        clSetKernelArg(testCuckooKernel, 5, cuckooHashService.getHashParams().getMem());
        clSetKernelArg(testCuckooKernel, 6, cuckooHashService.getStashUsed());
