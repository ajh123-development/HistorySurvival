...
public class CuckooHashOpenCLService {
    ...
    public int insertKeys(BufferGpu d_keys, int count) {
        ...
        CLKernel k_InsertKeys = clCreateKernel(kernelProgram.getKernel(KernelNames.CUCKOO), "Cuckoo_InsertKeys", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        ...
        do {
            if (insertedCount > 0) {
                ...
                int[] params = initHashValues();
                int errcode = CL10.clEnqueueWriteBuffer(ctx.getClQueue(), hashParams.getMem(), 0, 0, OCLUtils.getIntBuffer(params), null, null);
                OCLUtils.checkCLError(errcode);
                fillBufferLong(kernelProgram.getKernel(KernelNames.CUCKOO), ctx.getClQueue(), table, prime, meshGen.CUCKOO_EMPTY_VALUE, ctx.getErrcode_ret());
                fillBufferLong(kernelProgram.getKernel(KernelNames.CUCKOO), ctx.getClQueue(), stash, meshGen.CUCKOO_STASH_SIZE, meshGen.CUCKOO_EMPTY_VALUE, ctx.getErrcode_ret());
            }

            clSetKernelArg(k_InsertKeys, 0, d_keys.getMem());
            clSetKernelArg(k_InsertKeys, 1, table.getMem());
            clSetKernelArg(k_InsertKeys, 2, stash.getMem());
            clSetKernelArg(k_InsertKeys, 3, prime);
            clSetKernelArg(k_InsertKeys, 4, hashParams.getMem());
            clSetKernelArg(k_InsertKeys, 5, d_inserted.getMem());
            clSetKernelArg(k_InsertKeys, 6, d_stashUsed.getMem());
    ...
    private void fillBufferLong(CLProgram kernelProgram, CLCommandQueue clQueue, BufferGpu buffer, int count, long value, IntBuffer errcode_ret) {
        CLKernel fillBufferKernel = clCreateKernel(kernelProgram, "FillBufferLong", errcode_ret);
