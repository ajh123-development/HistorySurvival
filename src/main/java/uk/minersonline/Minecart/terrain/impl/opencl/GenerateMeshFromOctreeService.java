...
import uk.minersonline.Minecart.terrain.impl.GPUDensityField;
import uk.minersonline.Minecart.terrain.impl.GpuOctree;
...
public class GenerateMeshFromOctreeService {
    ...
    public int generateMeshKernel(KernelsHolder kernels, int[] meshIndex, int[] trianglesValid) {
        ...
        CLKernel k_GenerateMeshKernel = clCreateKernel(kernels.getKernel(KernelNames.OCTREE), "GenerateMesh", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        clSetKernelArg(k_GenerateMeshKernel, 0, octree.getNodeCodesBuf().getMem());
        clSetKernelArg(k_GenerateMeshKernel, 1, octree.getNodeMaterialsBuf().getMem());
        clSetKernelArg(k_GenerateMeshKernel, 2, d_indexBuffer.getMem());
        clSetKernelArg(k_GenerateMeshKernel, 3, d_trianglesValid.getMem());

        clSetKernelArg(k_GenerateMeshKernel, 4, octreeHashTable.getTable().getMem());
        clSetKernelArg(k_GenerateMeshKernel, 5, octreeHashTable.getStash().getMem());
        clSetKernelArg(k_GenerateMeshKernel, 6, octreeHashTable.getPrime());
        clSetKernelArg(k_GenerateMeshKernel, 7, octreeHashTable.getHashParams().getMem());
        clSetKernelArg(k_GenerateMeshKernel, 8, octreeHashTable.getStashUsed());
    ...
    public void compactMeshTrianglesKernel(KernelsHolder kernels, int numTriangles, int[] compactIndexBuffer){
        ...
        CLKernel k_CompactMeshTrianglesKernel = clCreateKernel(kernels.getKernel(KernelNames.OCTREE), "CompactMeshTriangles", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        clSetKernelArg(k_CompactMeshTrianglesKernel, 0, d_trianglesValid.getMem());
        clSetKernelArg(k_CompactMeshTrianglesKernel, 1, d_trianglesScan.getMem());
        clSetKernelArg(k_CompactMeshTrianglesKernel, 2, d_indexBuffer.getMem());
        clSetKernelArg(k_CompactMeshTrianglesKernel, 3, d_compactIndexBuffer.getMem());
    ...
    public int run(KernelsHolder kernels, int clipmapNodeSize) {
        ...
        float [] colourArray = {colour.X, colour.Y, colour.Z, 0.0f};
        FloatBuffer colourBuff = FloatBuffer.wrap(colourArray);

        CLKernel k_GenerateMeshVertexBufferKernel = clCreateKernel(kernels.getKernel(KernelNames.OCTREE), "GenerateMeshVertexBuffer", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        clSetKernelArg(k_GenerateMeshVertexBufferKernel, 0, octree.getVertexPositionsBuf().getMem());
        clSetKernelArg(k_GenerateMeshVertexBufferKernel, 1, octree.getVertexNormalsBuf().getMem());
        clSetKernelArg(k_GenerateMeshVertexBufferKernel, 2, octree.getNodeMaterialsBuf().getMem());
        clSetKernelArg(k_GenerateMeshVertexBufferKernel, 3, colourBuff);
        clSetKernelArg(k_GenerateMeshVertexBufferKernel, 4, d_vertexBuffer.getMem());
    ...
    public int findSeamNodesKernel(KernelsHolder kernels, int[] isSeamNode) {
        ...
        CLKernel k_FindSeamNodesKernel = clCreateKernel(kernels.getKernel(KernelNames.OCTREE), "FindSeamNodes", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        clSetKernelArg(k_FindSeamNodesKernel, 0, octree.getNodeCodesBuf().getMem());
        clSetKernelArg(k_FindSeamNodesKernel, 1, d_isSeamNode.getMem());
    ...
    public int gatherSeamNodesFromOctree(KernelsHolder kernels, Vec3i chunkMin, int chunkSize, List<OctreeNode> seamNodes, int numSeamNodes) {
        ...
        CLKernel k_ExtractSeamNodeInfoKernel = clCreateKernel(kernels.getKernel(KernelNames.OCTREE), "ExtractSeamNodeInfo", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 0, d_isSeamNode.getMem());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 1, d_isSeamNodeScan.getMem());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 2, octree.getNodeCodesBuf().getMem());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 3, octree.getNodeMaterialsBuf().getMem());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 4, octree.getVertexPositionsBuf().getMem());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 5, octree.getVertexNormalsBuf().getMem());
        clSetKernelArg(k_ExtractSeamNodeInfoKernel, 6, d_seamNodeInfo.getMem());
