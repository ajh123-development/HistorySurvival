...
public final class OpenCLCalculateMaterialsService {
    ...
    public void run(KernelsHolder kernels, int[] materials) {
        ...
        CLKernel clKernel = clCreateKernel(kernels.getKernel(KernelNames.DENSITY), "GenerateDefaultField", ctx.getErrcode_ret());
        ...
        clSetKernelArg(clKernel, 0, fieldBuff);
        clSetKernelArg(clKernel, 1, sampleScale);
        clSetKernelArg(clKernel, 2, ctx.defaultMaterial);
        clSetKernelArg(clKernel, 3, field.getMaterials().getMem());
    ...
    private void getResults(int[] result, GPUDensityField field) {
        if(result!=null) {
            ...
            CL10.clEnqueueReadBuffer(ctx.getClQueue(), field.getMaterials().getMem(), 1, 0, resultBuff, null, null);
