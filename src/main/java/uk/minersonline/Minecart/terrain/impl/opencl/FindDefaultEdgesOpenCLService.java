...
public final class FindDefaultEdgesOpenCLService {
    ...
    public int findFieldEdgesKernel(KernelsHolder kernels, int hermiteIndexSize, int[] edgeOccupancy, int[] edgeIndicesNonCompact) {
        ...
        CLKernel findFieldEdgesKernel = clCreateKernel(kernels.getKernel(KernelNames.FIND_DEFAULT_EDGES), "FindFieldEdges", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());
        clSetKernelArg(findFieldEdgesKernel, 0, field.getMaterials().getMem());
        clSetKernelArg(findFieldEdgesKernel, 1, edgeOccupancyBuffer.getMem());
        clSetKernelArg(findFieldEdgesKernel, 2, edgeIndicesNonCompactBuffer.getMem());
    ...
    public void compactEdgeKernel(KernelsHolder kernels, GPUDensityField field){
        ...
        CLKernel compactEdgesKernel = clCreateKernel(kernels.getKernel(KernelNames.FIND_DEFAULT_EDGES), "CompactEdges", ctx.getErrcode_ret());
        OCLUtils.checkCLError(ctx.getErrcode_ret());

        clSetKernelArg(compactEdgesKernel, 0, edgeOccupancyBuffer.getMem());
        clSetKernelArg(compactEdgesKernel, 1, edgeScanBuffer.getMem());
        clSetKernelArg(compactEdgesKernel, 2, edgeIndicesNonCompactBuffer.getMem());
        clSetKernelArg(compactEdgesKernel, 3, compactActiveEdgesBuffer.getMem());
    ...
    public void FindEdgeIntersectionInfoKernel(KernelsHolder kernels, Vec4f[] normals) {
        ...
        CLKernel kFindInfoKernel = clCreateKernel(kernels.getKernel(KernelNames.FIND_DEFAULT_EDGES), "FindEdgeIntersectionInfo", ctx.getErrcode_ret());
        int [] fieldArray = {
        ...
        clSetKernelArg(kFindInfoKernel, 0, fieldBuff);
        clSetKernelArg(kFindInfoKernel, 1, sampleScale);
        clSetKernelArg(kFindInfoKernel, 2, field.getEdgeIndices().getMem());
        clSetKernelArg(kFindInfoKernel, 3, field.getNormals().getMem());
